...
void amr::stage::Base::publish(ros::Time timestamp)
{
  ...
  {
	// Reimplementation of global velocity from Stg::ModelPosition::GetGlobalVelocity()
	// Compute global velocity
	Stg::Velocity gv;
	Stg::Velocity velocity = model_->GetVelocity();
	Stg::Pose gpose = model_->GetGlobalPose();
	...
	// New version
    nav_msgs::Odometry msg = createMessage(model_->GetGlobalPose(), gv);
